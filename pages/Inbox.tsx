    ...
    const renderTimelineItem = (item: TimelineItem, index: number, isLast: boolean) => {
        ...
        return (
          <div key={item.id} className="relative pb-10">
            {/* Timeline Vertical Line - 20px Axis Lock (Left margin 20px - 1px width = 19px) */}
            {!isLast && <div className={`absolute left-[19px] top-8 bottom-0 w-[2px] ${isCompleted ? 'bg-emerald-100' : isActive ? 'bg-indigo-50' : 'bg-slate-100'}`}></div>}
            
            {/* Marker - Center at 20px */}
            <div className="absolute left-0 top-1.5 w-10 h-10 flex items-center justify-center z-10">
                {isCompleted ? (
                    <div className="w-6 h-6 rounded-full bg-emerald-50 border border-emerald-200 flex items-center justify-center text-emerald-500 shadow-sm"><CheckCircle2 size={14} strokeWidth={3} /></div>
                ) : isActive ? (
                    <div className="relative flex items-center justify-center">
                        <div className="w-7 h-7 rounded-full border border-indigo-100 bg-white shadow-sm flex items-center justify-center">
            ...
            <div className="pl-14">
                ...
                <div className="flex justify-between items-center mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <h3 className={`text-sm font-black ${isActive ? 'text-slate-800' : 'text-slate-400'}`}>{item.title}</h3>
                    {item.monthTag && <span className={`text-[9px] px-1.5 py-0.5 rounded-md font-bold border ${isActive ? 'bg-indigo-50 text-indigo-500 border-indigo-100' : 'bg-slate-50 text-slate-400 border-slate-100'}`}>{item.monthTag}</span>}
                  </div>
                  {hasTask && <span className="text-[10px] font-black text-emerald-600 bg-emerald-50/80 px-2 py-1 rounded-lg flex items-center gap-1 border border-emerald-100/50 shadow-sm"><Play size={8} fill="currentColor"/> 待确认</span>}
                </div>
                                ...
                                <div className="flex gap-6">
                                    {item.metrics.map((m, idx) => (
                                        <div key={idx} className="flex flex-col justify-center">
                                            ...
                                            <p className={`text-base font-black font-mono tracking-tight ${isActive ? 'text-slate-800' : 'text-slate-400'}`}>{m.value}</p>
                                        </div>
                                    ))}
                                </div>
                                {hasTask && <div className="bg-emerald-600 text-white px-3 py-2 rounded-xl text-[11px] font-bold shadow-lg shadow-emerald-200/50 flex items-center gap-1 hover:bg-emerald-700 transition-colors">确认发放<ArrowRight size={12} strokeWidth={2.5} /></div>}
                            </div>
                        )}
                        {item.subItems && (
    ...
    return (
        <div className="flex flex-col h-full bg-[#F8F9FB]">
            <header className="sticky top-0 z-40 px-6 pt-12 pb-5 bg-[#F8F9FB]/90 backdrop-blur-xl border-b border-slate-100/50 flex justify-between items-end">
                ...
                <div className="w-10 h-10 rounded-full bg-white flex items-center justify-center border border-slate-100 shadow-sm text-slate-400 hover:text-slate-600 transition-colors"><MoreHorizontal size={20} /></div>
            </header>
            
            <div className="px-6 space-y-6 mt-6 pb-24 no-scrollbar overflow-y-auto">
                 {/* Timeline Section */}
                 <section className="bg-white rounded-[32px] shadow-[0_8px_30px_rgba(0,0,0,0.04)] border border-slate-100 overflow-hidden">
                    <div className="px-6 py-5 flex justify-between items-center cursor-pointer hover:bg-slate-50/50 transition-colors" onClick={() => setIsTimelineExpanded(!isTimelineExpanded)}>
                        <div className="flex items-center gap-4">
                            <div className="w-11 h-11 rounded-2xl bg-indigo-50 text-indigo-500 flex items-center justify-center border border-indigo-100/50"><Activity size={20} strokeWidth={2} /></div>
                            <div><h2 className="font-black text-slate-800 text-sm tracking-tight">交付履约进度</h2><p className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.1em]">Service Milestones</p></div>
                        </div>
                        <div className={`p-2 rounded-full transition-all duration-300 ${isTimelineExpanded ? 'rotate-180 bg-slate-100 text-slate-600' : 'text-slate-300'}`}><ChevronDown size={18} /></div>
                    </div>
