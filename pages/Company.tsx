    ...
    <div className="flex flex-col h-full bg-[#F8F9FB]">
      <header className="sticky top-0 z-40 px-6 pt-12 pb-5 bg-[#F8F9FB]/90 backdrop-blur-xl border-b border-slate-100/50 flex justify-between items-end">
          <div>
              ...
              <h1 className="text-2xl font-bold text-slate-800 tracking-tight">我的企业</h1>
          </div>
      </header>

      <div className="px-5 space-y-6 mt-6 pb-24 overflow-y-auto no-scrollbar">
        {/* Identity Card - Subtle Indigo/White Gradient */}
        <div className="bg-gradient-to-br from-white via-white to-indigo-50/40 rounded-[32px] p-6 shadow-[0_8px_30px_rgba(0,0,0,0.04)] border border-indigo-50/50 relative overflow-hidden group">
             <div className="flex items-start gap-5">
                 ...
                 <div className="pt-1 flex-1 min-w-0">
                     <div className="flex items-center justify-between mb-1.5">
                        <h2 className="text-lg font-bold text-slate-900 truncate tracking-tight">千机科技</h2>
                        <QrCode size={18} className="text-slate-300 hover:text-indigo-600 transition-colors cursor-pointer" />
                     </div>
                     ...
                     <div className="flex items-center justify-between bg-white/60 px-4 py-2.5 rounded-xl border border-slate-100/50 group-hover:border-indigo-100 transition-colors backdrop-blur-sm">
        ...
        {/* Bank Account Card - Refreshing 'Morning Mist' Gradient */}
        <div className="relative">
            ...
            <div className="bg-gradient-to-br from-white via-[#f8fbff] to-[#eef6ff] rounded-[32px] p-6 shadow-[0_20px_40px_-12px_rgba(59,130,246,0.1)] relative overflow-hidden group active:scale-[0.99] transition-all cursor-pointer border border-blue-100/60">
                ...
                <div className="relative z-10">
                    ...
                    <div className="flex items-center justify-between border-t border-slate-100/50 pt-5">
                        ...
                        <div className="flex items-center gap-1.5 text-[10px] text-slate-400 font-bold bg-white/60 px-2.5 py-1.5 rounded-lg border border-slate-50/50 shadow-sm backdrop-blur-md">
        ...
        {/* Menu Groups - Cleaner List */}
        <div className="space-y-6">
            {menuGroups.map((group) => (
                <div key={group.title}>
                    <h3 className="text-xs font-bold text-slate-400 mb-3 ml-2 uppercase tracking-wider">{group.title}</h3>
                    <div className="bg-white rounded-[28px] shadow-[0_4px_20px_rgba(0,0,0,0.02)] border border-slate-100 overflow-hidden">
                        {group.items.map((item, idx) => (
                            ...
                            >
                                <div className="flex items-center gap-4 overflow-hidden">
                                    <div className={`w-10 h-10 rounded-2xl flex items-center justify-center shrink-0 bg-slate-50 group-hover:bg-white group-hover:shadow-sm border border-transparent group-hover:border-slate-100 transition-all ${item.iconTheme}`}>
        ...
        <div className="text-center pt-4 pb-8 opacity-40">
            ...
            <p className="text-[9px] text-slate-400 font-mono">GM Pilot v1.0.14.225</p>
        </div>
      </div>
    </div>
  );
