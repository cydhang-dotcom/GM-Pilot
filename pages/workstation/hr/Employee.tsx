...
export const MOCK_EMPLOYEES = [
  ...
  { id: 5, name: '赵六', dept: '行政部', role: '行政主管', status: '离职', joinDate: '2020-05-20', leaveDate: '2023-08-15', phone: '13599998888', email: 'zhaoliu@company.com', idCard: '31010419880101****', contractEnd: '2023-12-05', tasks: { social: 'processing', fund: 'processing' } },
  { id: 6, name: '周晓', dept: '技术部', role: '测试工程师', status: '试用', joinDate: '2023-12-01', phone: '18800001111', email: 'zhouxiao@company.com', idCard: '31010419980101****', contractEnd: '2026-12-01', tasks: { social: 'pending', fund: 'pending' }, onboardingTasks: { info: 'done', contract: 'done', tax: 'processing', social: 'pending' } },
  ...
  { id: 9, name: '郑八', dept: '运营部', role: '运营专员', status: '离职', joinDate: '2021-08-10', leaveDate: '2024-01-10', phone: '13911113333', email: 'zhengba@company.com', idCard: '31010419910101****', contractEnd: '2024-08-10', tasks: { social: 'done', fund: 'done' } },
];
...
const Employee: React.FC = () => {
    ...
    const [filterMode, setFilterMode] = useState<'all' | 'active' | 'new' | 'leaving' | 'probation'>('active');
    ...
    const stats = useMemo(() => {
        ...
        const leaving = employees.filter(e => e.status === '离职' && (e.leaveDate?.startsWith('2023-12') || e.leaveDate?.startsWith('2024-01')));
    ...
    return (
        <div className="space-y-6 animate-fade-in pb-24">
            ...
            <div className="bg-white rounded-[32px] p-6 border border-slate-100 shadow-[0_8px_30px_rgba(0,0,0,0.04)] relative overflow-hidden">
                ...
                <div className="relative z-10">
                    ...
                    <div 
                        ...
                        className={`inline-flex items-baseline gap-2 mb-8 pl-1 cursor-pointer transition-all active:scale-95 ${filterMode === 'active' ? 'opacity-100' : 'opacity-40 hover:opacity-100'}`}
                    >
                        ...
                        {filterMode === 'active' && <div className="w-1.5 h-1.5 rounded-full bg-indigo-500 animate-pulse ml-1 mb-2"></div>}
                    </div>
                    ...
                    <div className="grid grid-cols-3 gap-3">
                        ...
                        <div 
                            onClick={() => setFilterMode(filterMode === 'leaving' ? 'all' : 'leaving')}
                            className={`rounded-2xl p-3 border transition-all cursor-pointer active:scale-95 flex flex-col items-center justify-center gap-1 ${
                                filterMode === 'leaving' 
                        ...
                        <div 
                            onClick={() => setFilterMode(filterMode === 'probation' ? 'all' : 'probation')}
                            className={`rounded-2xl p-3 border transition-all cursor-pointer active:scale-95 flex flex-col items-center justify-center gap-1 ${
            ...
            <div className="flex gap-3 px-1">
                <div className="bg-white flex-1 p-2.5 rounded-[20px] border border-slate-200 shadow-sm flex items-center gap-2 transition-all focus-within:border-indigo-300 focus-within:ring-4 focus-within:ring-indigo-50/50">
                    ...
                    <input 
                        type="text" 
                        placeholder={filterMode === 'all' ? "搜索姓名、部门..." : "搜..."} 
                        className="flex-1 text-sm outline-none text-slate-700 placeholder-slate-300 font-bold bg-transparent min-w-[50px] ml-1"
                        value={searchQuery}
                    ...
                    {searchQuery && (
                        <button 
                            ...
                            className="p-1 mr-1 text-slate-300 hover:text-slate-500 transition-colors shrink-0"
                        >
                            <XCircle size={16} strokeWidth={2.5} />
                        </button>
                    )}
                </div>
                <button className="bg-white w-[46px] rounded-[20px] border border-slate-200 shadow-sm flex items-center justify-center text-slate-400 active:scale-95 active:bg-slate-50 transition-all shrink-0">
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <div className="flex flex-col gap-0.5">
                                ...
                                <p className="text-[10px] font-bold font-mono text-slate-300 flex items-center gap-2">
                                    ...
                                    <span>{emp.status === '离职' ? '离职日期' : '入职日期'}: {emp.status === '离职' ? (emp.leaveDate || emp.contractEnd) : emp.joinDate}</span>
                                </p>
                            </div>
                        </div>
